...
12. [关键类索引](#12-关键类索引)
13. [深度专题](#13-深度专题)
    - [13.1 时间轮（Time Ring）](#131-时间轮time-ring)
    - [13.2 一致性 Hash 路由](#132-一致性-hash-路由)
    - [13.3 失败重试机制](#133-失败重试机制)
    - [13.4 LFU / LRU 路由](#134-lfu--lru-路由)
    - [13.5 任务结果丢失检测（findLostJobIds）](#135-任务结果丢失检测findlostjobids)
    - [13.6 路由策略全景（ROUND / FAILOVER / BUSYOVER 等）](#136-路由策略全景round--failover--busyover-等)
    - [13.7 分片广播（SHARDING_BROADCAST）](#137-分片广播sharding_broadcast)
    - [13.8 执行器回调可靠性（TriggerCallbackThread）](#138-执行器回调可靠性triggercallbackthread)
    - [13.9 阻塞策略（ExecutorBlockStrategyEnum）](#139-阻塞策略executorblockstrategyenum)
    - [13.10 日志报表与清理（JobLogReportHelper）](#1310-日志报表与清理joblogreporthelper)
    - [13.11 GLUE 动态任务](#1311-glue-动态任务)
    - [13.12 CronExpression 与调度类型](#1312-cronexpression-与调度类型)
    - [13.13 任务生命周期（CRUD / 启停 / 手动触发）](#1313-任务生命周期crud--启停--手动触发)
    - [13.14 @XxlJob Handler 注册机制](#1314-xxljob-handler-注册机制)
    - [13.15 执行器注册中心](#1315-执行器注册中心)
    - [13.16 OpenAPI 与双向通信协议](#1316-openapi-与双向通信协议)
    - [13.17 子任务编排（childJobId）](#1317-子任务编排childjobid)
    - [13.18 人工终止与失败告警](#1318-人工终止与失败告警)
    - [13.19 Web 认证与权限（xxl-sso）](#1319-web-认证与权限xxl-sso)
    - [13.20 实时日志滚动拉取](#1320-实时日志滚动拉取)
    - [13.21 执行上下文（XxlJobContext / XxlJobHelper）](#1321-执行上下文xxljobcontext--xxljobhelper)
    - [13.22 Admin 集群部署与 HA](#1322-admin-集群部署与-ha)
    - [13.23 与 Quartz / Spring @Scheduled 对比](#1323-与-quartz--spring-scheduled-对比)
    - [13.24 核心配置项参考](#1324-核心配置项参考)
    - [13.25 深度专题索引总表](#1325-深度专题索引总表)
    - [13.26 Spring AI 与 Dify 接入指南](#1326-spring-ai-与-dify-接入指南)
    - [13.27 Dashboard 仪表盘与统计口径](#1327-dashboard-仪表盘与统计口径)
    - [13.28 MethodJobHandler 与 JobThread 执行全链路](#1328-methodjobhandler-与-jobthread-执行全链路)
    - [13.29 性能调优实践清单](#1329-性能调优实践清单)
    - [13.30 常见问题排障手册](#1330-常见问题排障手册)
    - [13.31 无框架执行器（Frameless）接入](#1331-无框架执行器frameless接入)
    - [13.32 数据库表结构与关键字段](#1332-数据库表结构与关键字段)
    - [13.33 安全模型（AccessToken 双层鉴权）](#1333-安全模型accesstoken-双层鉴权)
    - [13.34 内置通用 Handler（HTTP / 命令行 / 分片）](#1334-内置通用-handlerhttp--命令行--分片)
    - [13.35 版本升级与迁移指南](#1335-版本升级与迁移指南)
    - [13.36 触发类型与 trigger_msg 解读](#1336-触发类型与-trigger_msg-解读)
...
`xxl-job-executor-sample-springboot-ai` 集成 Spring AI（Ollama/OpenAI）和 Dify，展示 XXL-JOB 作为 **AI 任务编排入口** 的扩展能力。详细接入步骤见 [§13.26](#1326-spring-ai-与-dify-接入指南)。
...
本章节对 **时间轮**、**一致性 Hash**、**失败重试** 三个机制做独立的源码级深析，包含数据结构、算法步骤、边界处理、与周边组件的协作关系，以及设计取舍。
...
- 详见 [13.3](#133-失败重试机制) 中与 Misfire 的区分
...
#### 13.2.4 MD5 Hash 函数（为何不用 hashCode）
...
#### 13.2.8 与 FAILOVER / BUSYOVER 的对比
...
#### 13.3.2 监控线程：JobFailAlarmMonitorHelper
...
#### 13.3.3 什么样的 log 算「失败」？（findFailJobLogIds SQL）
...
#### 13.3.9 告警联动（JobAlarmer + EmailJobAlarm）
...
#### 13.4.2 LFU：最不经常使用（ExecutorRouteLFU）
...
#### 13.4.3 LRU：最近最久未使用（ExecutorRouteLRU）
...
#### 13.4.5 与 Executor 端 JobThread 的配合
...
丢失标记 `handle_code=500` 且 `alarm_status` 仍为 **0** → 会被 `JobFailAlarmMonitorHelper.findFailJobLogIds` 扫到 → 若配置了 `executor_fail_retry_count > 0`，会 **RETRY 重新触发**（见 §13.3）。
...
#### 13.5.10 与 findFailJobLogIds 的协作关系
...
### 13.6 路由策略全景（ROUND / FAILOVER / BUSYOVER 等）

前文章节已深析 CONSISTENT_HASH（§13.2）、LFU/LRU（§13.4）。本节补齐其余策略源码，并给出 **10 种路由统一对比**。
...
### 13.8 执行器回调可靠性（TriggerCallbackThread）

§13.5 讲了 Admin 侧「丢失检测」；本节从 **Executor 侧** 分析回调如何尽力送达，以及与丢失检测的竞态关系。
...
#### 13.8.4 doCallback：多 Admin  failover
...
#### 13.8.8 与 findLostJobIds 的竞态时间线
...
| Admin 丢失检测 | 最终兜底标记失败（§13.5） |

---

### 13.9 阻塞策略（ExecutorBlockStrategyEnum）
...
#### 13.9.5 与 Admin 失败重试的关系

`DISCARD_LATER` 导致 trigger 失败 → `trigger_code=500` → 可能触发 **Admin RETRY**（§13.3），与阻塞策略叠加可能反复 discard，需在任务配置中协调「重试次数」与「阻塞策略」。

---

### 13.10 日志报表与清理（JobLogReportHelper）
...
#### 13.11.3 Executor 侧：ExecutorBizImpl 分支
...
#### 13.11.4 Groovy GLUE（GlueFactory）
...
#### 13.11.5 脚本 GLUE（ScriptJobHandler + ScriptUtil）
...
### 13.12 CronExpression 与调度类型

#### 13.12.1 调度类型枚举（ScheduleTypeEnum）
...
#### 13.15.2 Executor 侧：ExecutorRegistryThread
...
#### 13.15.4 监控线程：registryMonitorThread
...
T=90s   findDead 删除 → findAll 不可见 → group.address_list 移除
        → findLostJobIds 可能命中（§13.5）
```

#### 13.15.5 与 JobTrigger 的衔接
...
#### 13.16.2 Admin OpenAPI（OpenApiController）
...
#### 13.16.3 Executor HTTP 服务（EmbedServer）
...
#### 13.17.3 触发时机：JobCompleter.processChildJob
...
#### 13.18.1 人工终止 running 任务（logKill）
...
#### 13.18.2 与 COVER_EARLY / destroy 对比
...
**触发时机**：`JobFailAlarmMonitorHelper` 处理 fail log 时（§13.3），与 RETRY **同轮**执行。

#### 13.18.4 EmailJobAlarm 源码要点
...
**alarm_status**：2=邮件成功，3=邮件失败（§13.3 CAS 更新）。
...
### 13.21 执行上下文（XxlJobContext / XxlJobHelper）

业务代码通过 **ThreadLocal 上下文** 与框架交互，无需显式传参。

#### 13.21.1 XxlJobContext 字段
...
### 13.23 与 Quartz / Spring @Scheduled 对比
...
#### 13.24.1 Admin（application.properties）
...
#### 13.24.2 Executor（Spring Boot 示例）
...
| 章节 | 主题 | 核心类 / 入口 |
|------|------|---------------|
| §13.1 | 时间轮 | `JobScheduleHelper.ringData` |
| §13.2 | 一致性 Hash | `ExecutorRouteConsistentHash` |
| §13.3 | 失败重试 | `JobFailAlarmMonitorHelper` |
| §13.4 | LFU / LRU | `ExecutorRouteLFU/LRU` |
| §13.5 | 结果丢失 | `findLostJobIds` + `JobCompleteHelper` |
| §13.6 | 路由全景 | `ExecutorRouteStrategyEnum` |
| §13.7 | 分片广播 | `JobTrigger` SHARDING_BROADCAST |
| §13.8 | 回调可靠性 | `TriggerCallbackThread` |
| §13.9 | 阻塞策略 | `ExecutorBizImpl` + `ExecutorBlockStrategyEnum` |
| §13.10 | 日志报表 | `JobLogReportHelper` |
| §13.11 | GLUE 动态任务 | `GlueFactory` / `ScriptJobHandler` |
| §13.12 | Cron 解析 | `CronExpression` / `ScheduleTypeEnum` |
| §13.13 | 任务生命周期 | `XxlJobServiceImpl` start/stop/trigger |
| §13.14 | Handler 注册 | `XxlJobSpringExecutor` + `@XxlJob` |
| §13.15 | 执行器注册 | `JobRegistryHelper` + `ExecutorRegistryThread` |
| §13.16 | OpenAPI 协议 | `OpenApiController` + `EmbedServer` |
| §13.17 | 子任务 | `JobCompleter.processChildJob` |
| §13.18 | kill / 告警 | `JobLogController.logKill` + `EmailJobAlarm` |
| §13.19 | Web 权限 | `XxlSsoConfig` + `JobGroupPermissionUtil` |
| §13.20 | 实时日志 | `logDetailCat` + `XxlJobFileAppender.readLog` |
| §13.21 | 执行上下文 | `XxlJobContext` / `XxlJobHelper` |
| §13.22 | Admin 集群 HA | `schedule_lock` + 组件集群行为 |
| §13.23 | 框架对比 | XXL-JOB vs Quartz vs @Scheduled |
| §13.24 | 配置参考 | application.properties |
| §13.25 | 本索引 | — |
| §13.26 | Spring AI / Dify | `AIXxlJob` + sample-springboot-ai |
| §13.27 | Dashboard 统计 | `dashboardInfo` / `chartInfo` |
| §13.28 | Handler 执行链 | `MethodJobHandler` + `JobThread` |
| §13.29 | 性能调优 | trigger 池 / batchsize / 索引 |
| §13.30 | 排障手册 | 现象 → 根因 → 处理 |
| §13.31 | Frameless 接入 | `XxlJobSimpleExecutor` |
| §13.32 | 数据库表结构 | `tables_xxl_job.sql` |
| §13.33 | 安全模型 | AccessToken + xxl-sso |
| §13.34 | 通用 Handler | `SampleXxlJob` |
| §13.35 | 版本升级 | 2.x → 3.4.x |
| §13.36 | 触发类型 | `TriggerTypeEnum` + trigger_msg |

**推荐阅读路径**：

1. **调度内核**：§13.1 → §13.12 → §13.13  
2. **分布式执行**：§13.6 → §13.7 → §13.15 → §13.16  
3. **可靠性**：§13.3 → §13.5 → §13.8 → §13.18  
4. **运维排障**：§13.30 → §13.20 → §13.18 → §13.24 → §13.22  
5. **AI 任务接入**：§13.26  
6. **执行器开发**：§13.14 → §13.28 → §13.31 → §13.21  
7. **数据与安全**：§13.32 → §13.33 → §13.19  
8. **升级迁移**：§13.35 → §13.32  

---

### 13.26 Spring AI 与 Dify 接入指南

#### 13.26.1 定位：XXL-JOB 与 AI 的关系
...
#### 13.26.4 接入 Spring AI（Ollama 示例）
...
### 13.28 MethodJobHandler 与 JobThread 执行全链路

#### 13.28.1 从 @XxlJob 到 MethodJobHandler
...
#### 13.28.4 BEAN vs GLUE vs Script 在 JobThread 中的差异
...
| 运行中变失败 | `findLostJobIds` 标记丢失 | 回调超时 > 10min（默认逻辑见 §13.5） |
...
### 13.31 无框架执行器（Frameless）接入

#### 13.31.1 适用场景

| 场景 | 推荐集成方式 |
|------|--------------|
| Spring Boot 业务 | `XxlJobSpringExecutor` + `@XxlJob`（§13.14） |
...
#### 13.33.4 Executor EmbedServer 端点
...
### 13.34 内置通用 Handler（HTTP / 命令行 / 分片）
...
3. 执行增量 SQL（按源版本选择 §13.35.2 / §13.35.3）
...
#### 13.35.6 Docker Compose（v3.3.2+）

官方 `docker/` 目录提供 Compose 一键启 Admin 集群，适合 POC；生产仍建议自建 MySQL HA 与 Executor 部署。

---

### 13.36 触发类型与 trigger_msg 解读

#### 13.36.1 TriggerTypeEnum 全表
...
#### 13.36.2 JobTrigger.processTrigger 流程与日志
...
#### 13.36.4 排障时如何读一条 log

```
trigger_code=500  → 看 trigger_msg（Admin 侧即失败，Executor 未执行或拒绝）
trigger_code=200 & handle_code=0  → Executor 执行中或回调丢失（§13.5）
...
*文档生成时间：2026-05-29*  
*最后更新：2026-05-29（补充 §13.34~§13.36：通用 Handler、版本升级、触发类型）*  
